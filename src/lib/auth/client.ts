 ...
 *
 * Drop-in client for browser or Node.js apps to authenticate against
 * a FilyBase project's auth server. Zero dependencies.
 *
 * Usage:
 *   import { createFilyAuth } from "@/lib/auth/client";
 *   const auth = createFilyAuth({ baseUrl: "http://localhost:3000/api/auth/<projectId>" });
 *   const session = await auth.signUp({ email, password, name });
 */

export interface FilyAuthConfig {
  /** Full base URL to the project auth API, e.g. "http://localhost:3000/api/auth/abc123" */
  baseUrl: string;
...
export function createFilyAuth(config: FilyAuthConfig) {
  ...
  async function request<T>(
    ...
    try {
      ...
      if (!res.ok) {
        return { data: null, error: json as AuthError };
      }
  ...
  async function signUp(input: {
    ...
    const result = await request<AuthSession>("/signup", {
      method: "POST",
  ...
  async function signIn(input: {
    ...
    const result = await request<AuthSession>("/signin", {
      method: "POST",
      body: JSON.stringify(input),
    });
    if (result.data) saveSession(result.data);
  ...
  async function refreshSession(): Promise<Result<AuthSession>> {
    if (!currentSession?.refreshToken) {
      return {
        data: null,
        error: { error: "no_session", error_description: "No active session" },
      };
    }
    const result = await request<AuthSession>("/refresh", {
      method: "POST",
  ...
  async function getUser(): Promise<Result<Record<string, unknown>>> {
    if (!currentSession?.accessToken) {
      return {
        data: null,
        error: { error: "no_session", error_description: "Not authenticated" },
      };
    }
    return request("/user", {
      method: "GET",
